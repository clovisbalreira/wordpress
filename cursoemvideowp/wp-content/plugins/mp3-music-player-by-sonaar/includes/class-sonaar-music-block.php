    ...
    function render_sonaar_block( $attributes ) {
		...
		if( function_exists( 'run_sonaar_music_pro' ) ) { 
			...
			if( $title_soundwave_color != '' )  {
				$custom_css .= ' #'.$block_id .' .track-title, #'.$block_id .' .player, #'.$block_id .' .srp_player_boxed .album-title { color: '.$title_soundwave_color.'; }';
			}
			if( $title_soundwave_fontsize > 0 )  {
                $custom_css .=  ' #'.$block_id .' .iron-audioplayer .track-title, #'.$block_id .' .srp_player_boxed .album-title { font-size: '. $title_soundwave_fontsize .'px; }';
			}
