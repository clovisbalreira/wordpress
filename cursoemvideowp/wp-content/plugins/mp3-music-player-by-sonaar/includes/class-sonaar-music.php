...
class Sonaar_Music {
	...
	private function define_admin_hooks() {
		...
		if(class_exists('Iron_sonaar') || $plugin_admin->setPlaylistCPTName() === 'album'){ // If Sonaar Theme is activated
			$this->loader->add_action('manage_album_posts_custom_column', $plugin_admin , 'manage_album_custom_column', 10, 2);//initial cpt for retro compatibility
			$this->loader->add_filter('manage_album_posts_columns', $plugin_admin , 'manage_album_columns');//initial cpt for retro compatibility
		}
	...
	public static function convertSomeOptionValue($value){
		...
		if( is_array( $value ) && array_key_exists( 'sr_prevent_continuous_url',  $value ) ){
			...
			$pageListToAvoid = array_map(function($url) { return strval(url_to_postid($url)); }, $pageListToAvoid);
			$value['sr_prevent_continuous_url'] = $pageListToAvoid;
		}
